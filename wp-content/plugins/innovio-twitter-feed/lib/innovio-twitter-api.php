...
include_once 'innovio-twitter-helper.php';

/**
 * Class InnovioTwitterApi
 */
class InnovioTwitterApi {
	/**
	...
	const REQUEST_TOKEN_FIELD = 'innovio_twitter_request_token';
	const REQUEST_TOKEN_SECRET_FIELD = 'innovio_twitter_request_token_secret';
	const ACCESS_TOKEN_FIELD = 'innovio_twitter_access_token';
	const ACCESS_TOKEN_SECRET_FIELD = 'innovio_twitter_access_token_secret';
	const AUTHORIZE_TOKEN_FIELD = 'innovio_twitter_authorize_token';
	const AUTHORIZE_VERIFIER_FIELD = 'innovio_twitter_authorize_verifier';
	const USER_ID_FIELD = 'innovio_twitter_user_id';
	const USER_SCREEN_NAME_FIELD = 'innovio_twitter_screen_name';
	...
	public function __construct() {
		...
		$this->helper          = new InnovioTwitterHelper();
	}

	/**
	 * @return InnovioTwitterApi
	 */
	 ...
	 *
	 * @see InnovioTwitterApi::generateSignatureBase()
	 */
	...
	public function obtainRequestToken() {
		...
		if ( is_wp_error( $response ) ) {
			...
			$responseObj->message = esc_html__( 'Internal WP error', 'innovio-twitter-feed' );
		} else {
			...
			if ( ! empty( $responseBody ) ) {
				...
				if ( ( is_array( $responseParsed ) && count( $responseParsed ) ) &&
					...
					if ( ! empty( $responseObj->redirectUrl ) ) {
						$responseObj->status  = false;
						$responseObj->message = esc_html__( 'Redirect URL couldn\t not be generated', 'innovio-twitter-feed' );
					} else {
					...
					$responseObj->status  = false;
					$responseObj->message = esc_html__( 'Couldn\'t connect with Twitter API', 'innovio-twitter-feed' );
				}
			}
		}
		...
		if ( ! empty( $authorizeVerifier ) && ! empty( $authorizeToken ) ) {
			...
			if ( is_wp_error( $response ) ) {
				...
				if ( is_array( $responseParsed ) && count( $responseParsed )
					...
					$responseObj->status  = true;
					$responseObj->message = esc_html__( 'Access token obtained', 'innovio-twitter-feed' );
				}
			}
		} else {
			$responseObj->status  = false;
			$responseObj->message = esc_html__( 'Authorize token and it\'s secret were not obtainer', 'innovio-twitter-feed' );
		}
	...
	public function fetchTweets( $userId = '', $count = '', $transient = array() ) {
		...
		if ( ! $this->transientExists( $transient ) ) {
			if ( $userId && $accessToken ) {
				...
				if ( is_wp_error( $response ) ) {
					...
					if ( isset( $response['response']['code'] ) && $response['response']['code'] == 200 ) {
						...
						$responseObj->status  = false;
						$responseObj->message = esc_html__( 'Couldn\'t connect with Twitter', 'innovio-twitter-feed' );
					}
				}
			} else {
				$responseObj->status  = false;
				$responseObj->message = esc_html__( 'It seams like you haven\t connected with your Twitter account', 'innovio-twitter-feed' );
			}
		} else {
			...
			if ( ! empty( $transientContent ) ) {
				...
				$responseObj->status  = false;
				$responseObj->message = esc_html__( 'Couldn\'t retreive content from database', 'innovio-twitter-feed' );
			}
		}
