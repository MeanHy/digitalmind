...
function sb_load_wp_auth_key() {
    if ((!defined('SB_DB_PASSWORD') || empty(SB_DB_PASSWORD)) && !defined('AUTH_KEY') && !defined('DB_NAME') && file_exists(SB_PATH . '/../../../../wp-config.php')) {
        $config = file_get_contents(SB_PATH . '/../../../../wp-config.php');
        if (strpos($config, 'AUTH_KEY')) {
            $config = str_replace(['"', "'"], '', $config);
            $config = substr($config, strpos($config, 'AUTH_KEY') + 8);
            $config = substr($config, 0, strpos($config, ')'));
            if ($config) {
                define('AUTH_KEY', trim(str_replace(['"', "'", ','], '', $config)));
            }
        }
    }
}
