...
function sb_admin_action() {
    ...
    if (strpos(SB_URL, site_url()) === false) {
        $response = sb_installation(sb_installation_array(), true);
    }
    if ($response && ($response === true || empty($response['error']))) {
        ...
        echo '<script>const SB_AJAX_URL = "' . plugins_url() . '/supportboard/supportboard/include/ajax.php";const SB_URL = "' . site_url() . '";</script>';
...
function sb_enqueue() {
    ...
    if (sb_get_setting('wp-manual') !== true) {
        ...
        if ($exclusions[2] != false) {
            if (count($exclusions[0]) && (($exclusions[2] == 'show' && !in_array($page_id, $exclusions[0])) || ($exclusions[2] == 'hide' && in_array($page_id, $exclusions[0])))) {
                return false;
            }
            if (($exclusions[2] == 'show' && sb_get_multi_setting('wp-visibility', 'wp-visibility-logged-in') && !$current_user_id) || ($exclusions[2] == 'hide' && sb_get_multi_setting('wp-visibility', 'wp-visibility-logged-in') && $current_user_id)) {
                return false;
            }
        }
...
function sb_on_wp_user_update($user_id) {
    if (sb_get_setting('wp-users-system') == 'wp') {
        ...
        if (!$active_user) {
            ...
            if ($settings['email'] != $active_user['email'] && (sb_is_admin_page() || sb_is_agent() || sb_is_agent($settings))) {
                $active_user = false;
            }
        }
        if ($active_user) {
