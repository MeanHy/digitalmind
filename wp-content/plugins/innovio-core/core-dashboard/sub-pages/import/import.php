...
class InnovioCoreImport {
	...
	public function set_import_url() {
		$params = InnovioCoreDashboard::get_instance()->get_import_params();
	...
	public function import_action() {
					...
					switch ($_POST['options']['action']):
						...
						case 'rev-slider':
							if (innovio_core_is_revolution_slider_installed()) {
								...
								$this->set_message(esc_html__('Revolution Slider isn\'t installed', 'innovio-core'));
							}
							break;
						case 'content':
							...
							if($content_start) {
								if (!InnovioCoreDashboard::get_instance()->check_purchase_code($_POST['options']['demo'])) {
									innovio_core_ajax_status('error', esc_html__('Please don\'t try to hack me. Purchase code registered is not valid', 'innovio-core'));
									exit;
								}
							}
			...
			innovio_core_ajax_status($this->get_status(), $this->get_message(), $this->get_data());
		}
		wp_die();
	}
	...
	function file_content( $path ) {
        ...
        if ( 200 !== intval( $response_code ) ) {
			$this->set_message($response["response"]['message'] . ' ' . esc_html__('Please contact support', 'innovio-core'));
	...
	public function import_widgets($demo) {
		...
		if($widgets_content) {
			foreach ((array)$widgets_content['widgets'] as $innovio_widget_id => $innovio_widget_data) {
				update_option('widget_' . $innovio_widget_id, $innovio_widget_data);
			}
			$ws = $this->import_sidebars_widgets($widgets);
			if($ws) {
				$this->set_message(esc_html__('Widgets are set for proper sidebar', 'innovio-core'));
				$this->set_data('type', 'options');
				$this->set_status('success');
			}
		}
	...
	public function import_sidebars_widgets( $file ) {
		$innovio_sidebars = get_option( "sidebars_widgets" );
		unset( $innovio_sidebars['array_version'] );
		$data = $this->unserialized_content( $file );

		if ( $data && is_array( $data['sidebars'] ) ) {
			$innovio_sidebars = array_merge( (array) $innovio_sidebars, (array) $data['sidebars'] );
			unset( $innovio_sidebars['wp_inactive_widgets'] );
			$innovio_sidebars                  = array_merge( array( 'wp_inactive_widgets' => array() ), $innovio_sidebars );
			$innovio_sidebars['array_version'] = 2;
			wp_set_sidebars_widgets( $innovio_sidebars );
			return true;
		} else {
			return false;
		}
	...
	public function import_options( $file ) {
		...
		$current_options = get_option(INNOVIO_CORE_OPTIONS_NAME);
		if($options){
			if($current_options != $options) {
				$result = update_option(INNOVIO_CORE_OPTIONS_NAME, $options);
				if ($result) {
					$this->update_options_after_import($file);
					$this->set_status('success');
					$this->set_data('type', 'options');
					$this->set_message(esc_html__('Options imported successfully', 'innovio-core'));

					$this->update_options_after_import($file);

				} else {
					$this->set_status('error');
					$this->set_message(esc_html__('Problem occurred during options import', 'innovio-core'));
				}
			} else {
				$this->set_status('success');
				$this->set_data('type', 'options');
				$this->set_message(esc_html__('Options are already imported', 'innovio-core'));
			}
		}
	...
	public function import_settings_pages( $file ) {
		...
		$new_ids = get_transient( '_innovio_core_imported_posts' );
		$fields_status = true;
		...
		if($pages) {
			...
			if (!$fields_status) {
				$this->set_status('error');
				$this->set_message(esc_html__('Problem occurred during settings pages import', 'innovio-core'));
			} else {
				$this->set_status('success');
				$this->set_data('type', 'options');
				$this->set_message(esc_html__('Settings pages imported successfully', 'innovio-core'));
			}
		} else {
			$this->set_status('error');
			$this->set_message(esc_html__('File doesn\'t exist', 'innovio-core'));
		}
	}
	...
	public function import_menu_settings( $file ) {
		...
		if($menus_data) {
			...
			$this->set_status('success');
			$this->set_data('type', 'options');
			$this->set_message( esc_html__( 'Menus set for proper locations', 'innovio-core' ) );
		} else {
			$this->set_status('error');
			$this->set_message( esc_html__( 'Problem during menus location set', 'innovio-core' ) );
		}
	}
	...
	public function import_content( $file, $xml, $attachments, $post_id) {
		ob_start();
		require_once( INNOVIO_CORE_ABS_PATH . '/core-dashboard/sub-pages/import/wordpress-importer.php' );
...
//		}
//
		if(innovio_core_is_woocommerce_installed()) {
		...
		$innovio_import = new WP_Import();
		set_time_limit( 0 );

		$innovio_import->fetch_attachments = $attachments;
		$returned_value                  = $innovio_import->import( $file . $xml );
		...
		if ( is_wp_error( $returned_value ) ) {
			...
			$this->set_message( esc_html__( 'An error occurred during content import', 'innovio-core' ) );
		} else {
			$this->set_status('success');
			$this->set_data('type', 'content');
			$this->set_data('posts', $this->imported_posts);
			$this->set_message( esc_html__( 'File imported successfully', 'innovio-core' ) . ' ' . $xml );
		...
		if($files_created) {
			...
			foreach ($rev_sliders as $rev_slider) {
				...
				if (!$rev_results['success']) {
					$this->set_status('error');
					$this->set_message(esc_html__('Error while importing rev sliders', 'innovio-core'));
					exit;
				}
			}
			$this->set_status('success');
			$this->set_data('type', 'options');
			$this->set_message(esc_html__('Rev sliders imported successfully', 'innovio-core'));
		} else {
			$this->set_status('error');
			$this->set_data('type', 'options');
			$this->set_message(esc_html__('Files don\'t exist', 'innovio-core'));
		}
	}
	...
	function update_options_after_import( $folder ) {
		...
		foreach ( $demo_urls as $demo_url ) {
			$global_options    = get_option( INNOVIO_CORE_OPTIONS_NAME );
			$new_global_values = str_replace( $demo_url, $url, $global_options );

			update_option( INNOVIO_CORE_OPTIONS_NAME, $new_global_values );
		}
	}
	...
	function get_noimage_url( $origin_img_url ) {
		...
		return INNOVIO_CORE_ASSETS_URL_PATH . '/img/' . $noimage_fname;
	}
	...
	public function populate_single_pages() {
		...
		if ( isset( $_POST ) && !empty( $_POST ) && !empty($_POST['options']['demo']) ) {
			if ( wp_verify_nonce( $_POST['options']['nonce'], 'mkdf_cd_import_nonce' ) ) {
				...
				$html = innovio_core_get_module_template_part('core-dashboard/sub-pages/import', 'pages-list', '', array('pages' => $pages));

				if($pages){
					innovio_core_ajax_status( 'success', '', $html);
				} else {
					innovio_core_ajax_status( 'error', esc_html__( 'Pages don\'t exist', 'innovio-core' ), '');
				}
			}
		}
	...
	public function is_ready_to_import() {
		$info = InnovioCoreSystemInfoPage::get_instance()->get_system_info();
...
InnovioCoreImport::get_instance();