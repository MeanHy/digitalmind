...
foreach ( glob( MIKADO_FRAMEWORK_ICONS_ROOT_DIR . '/*/load.php' ) as $module_load ) {
	if ( innovio_mikado_is_customizer_item_enabled( $module_load, 'innovio_performance_disable_icon_pack_' ) ) {
		include_once $module_load;
	}
}

if ( ! function_exists( 'innovio_mikado_get_icon_pack_collections' ) ) {
	function innovio_mikado_get_icon_pack_collections() {
		$options = apply_filters( 'innovio_mikado_filter_add_icon_pack_into_collection', $options = array() );
		
		return $options;
	}
	
	add_action( 'after_setup_theme', 'innovio_mikado_get_icon_pack_collections' );
}

if ( ! function_exists( 'innovio_mikado_get_icon_pack_options' ) ) {
	function innovio_mikado_get_icon_pack_options() {
		$options = apply_filters( 'innovio_mikado_filter_add_icon_pack_into_options', $options = array() );
		
		return $options;
	}
	
	add_action( 'after_setup_theme', 'innovio_mikado_get_icon_pack_options' );
}

if ( ! function_exists( 'innovio_mikado_get_icon_pack_social_options' ) ) {
	function innovio_mikado_get_icon_pack_social_options() {
		$options = apply_filters( 'innovio_mikado_filter_add_icon_pack_into_social_options', $options = array() );
		
		return $options;
	}
	
	add_action( 'after_setup_theme', 'innovio_mikado_get_icon_pack_social_options' );
}

/*
 * Class: InnovioMikadoClassIconCollections
 */
class InnovioMikadoClassIconCollections {
	...
	private function __construct() {
		...
		$this->iconPackCollections   = innovio_mikado_get_icon_pack_collections();
		$this->iconPackOptions       = innovio_mikado_get_icon_pack_options();
		$this->iconPackSocialOptions = innovio_mikado_get_icon_pack_social_options();
	...
	public function getIconsMetaBoxOrOption( $attributes ) {
		...
		foreach ( $icon_collections as $collection_key => $collection_object ) {
			...
			$icon_pack_container = innovio_mikado_add_admin_container(
				array(
					'parent'     => $parent,
	...
	public function getVCParamsArray( $iconPackDependency = array(), $iconCollectionPrefix = "", $emptyIconPack = false ) {
		...
		$iconPackParams = array(
			'type'        => 'dropdown',
			'heading'     => esc_html__( 'Icon Pack', 'innovio' ),
		...
		if ( is_array( $this->iconCollections ) && count( $this->iconCollections ) ) {
			foreach ( $this->iconCollections as $key => $collection ) {
				$iconSetParams[] = array(
					'type'        => 'dropdown',
					'heading'     => esc_html__( 'Icon', 'innovio' ),
	...
	public function getIconWidgetParamsArray() {
		$iconPackParams[] = array(
			'type'    => 'dropdown',
			'name'    => 'icon_pack',
			'title'   => esc_html__( 'Icon Pack', 'innovio' ),
			'options' => $this->getIconPackOptions()
		);
		...
		if ( is_array( $this->iconCollections ) && count( $this->iconCollections ) ) {
			foreach ( $this->iconCollections as $key => $collection ) {
				$iconSetParams[] = array(
					'type'    => 'dropdown',
					'title'   => $collection->title . esc_html__( ' Icon', 'innovio' ),
	...
	public function getSocialIconWidgetMultipleParamsArray( $count ) {
		...
		for ( $n = 1; $n <= $count; $n ++ ) {
			if ( is_array( $iconCollectionsVC ) && count( $iconCollectionsVC ) ) {
				foreach ( $iconCollectionsVC as $key => $collection ) {
					$iconOps[] = array(
						...
						'title'   => sprintf( esc_html__( 'Icon %s %s Icon', 'innovio' ), $n, $collection->title ),
			...
			$iconOps[] = array(
				'type'  => 'textfield',
				'name'  => 'link_' . $n,
				'title' => sprintf( esc_html__( 'Link %s', 'innovio' ), $n )
			);
			
			$iconOps[] = array(
				'type'    => 'dropdown',
				'name'    => 'target_' . $n,
				'title'   => sprintf( esc_html__( 'Link Target %s', 'innovio' ), $n ),
	...
	public function getIconSizesArray() {
		return array(
			esc_html__( 'Tiny', 'innovio' )       => 'fa-lg',
			esc_html__( 'Small', 'innovio' )      => 'fa-2x',
			esc_html__( 'Medium', 'innovio' )     => 'fa-3x',
			esc_html__( 'Large', 'innovio' )      => 'fa-4x',
			esc_html__( 'Very Large', 'innovio' ) => 'fa-5x'
		);
	}
	...
	public function getIconCollectionIcons( iInnovioMikadoInterfaceIconCollection $collection ) {
	...
	public function getIconCollectionsEmpty( $no_empty_key = "" ) {
		$array                  = array();
		$array[ $no_empty_key ] = esc_html__( 'No Icon', 'innovio' );
	...
	public function getIconCollectionsVCEmpty() {
		$vc_array                                           = array();
		$vc_array[ esc_html__( 'No Icon', 'innovio' ) ] = '';
	...
	public function enqueueStyles() {
		if ( is_array( $this->iconCollections ) && count( $this->iconCollections ) ) {
			foreach ( $this->iconCollections as $collection_key => $collection_obj ) {
				wp_enqueue_style( 'innovio-mikado-' . $collection_key, $collection_obj->styleUrl );
			}
		}
	}
