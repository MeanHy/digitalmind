...
/*
   Class: InnovioMikadoClassDashboardForm
   A class that initializes Mikado Dashboard Container
*/

class InnovioMikadoClassDashboardForm implements iInnovioMikadoInterfaceLayoutNode, iInnovioMikadoInterfaceRender {
	public $children;
	public $name;
	public $form_id;
	public $form_method;
	...
	public function render( $factory ) {
		...
		<form method="<?php echo esc_attr( $this->form_method ); ?>" id="<?php echo esc_attr( $this->form_id ); ?>" class="<?php echo esc_attr( $action_class ) ?>" data-action="<?php echo esc_attr( $this->form_action ); ?>">
			...
			if ( innovio_mikado_core_plugin_installed() ) {
				echo innovio_mikado_get_button_html( array(
					'html_type'    => 'button',
	...
	public function renderChild( iInnovioMikadoInterfaceRender $child, $factory ) {
		$child->render( $factory );
	}
}

/*
   Class: InnovioMikadoClassDashboardGroup
   A class that initializes InnovioMikadoClass Group Field
*/
class InnovioMikadoClassDashboardGroup implements iInnovioMikadoInterfaceLayoutNode, iInnovioMikadoInterfaceRender {
	public $children;
	public $name;
	public $title;
	public $description;
...
/*
   Class: InnovioMikadoClassDashboardTitle
   A class that initializes Dashboard Title
*/
class InnovioMikadoClassDashboardTitle implements iInnovioMikadoInterfaceRender {
	private $name;
	private $title;
	private $args = array();
...
/*
   Class: InnovioMikadoClassDashboardField
   A class that initializes InnovioMikadoClass Front Field
*/
class InnovioMikadoClassDashboardField implements iInnovioMikadoInterfaceRender {
	private $type;
	private $name;
	private $label;
	private $description;
...
abstract class InnovioMikadoClassDashboardFieldType {
	abstract public function render( $name, $label = "", $description = "", $options = array(), $args = array(), $value = "" );
}

class InnovioMikadoClassDashboardFieldText extends InnovioMikadoClassDashboardFieldType {
...
class InnovioMikadoClassDashboardFieldTextArea extends InnovioMikadoClassDashboardFieldType {
	public function render( $name, $label = "", $description = "", $options = array(), $args = array(), $value = '', $repeat = array() ) {
		$col_width = 12;
...
class InnovioMikadoClassDashboardFieldImage extends InnovioMikadoClassDashboardFieldType {
	public function render( $name, $label = "", $description = "", $options = array(), $args = array(), $value = '', $repeat = array() ) {
		$class = '';
			...
			<div class="mkdf-dashboard-gallery-holder">
				<ul class="mkdf-dashboard-gallery-images-holder">
					<?php if ( isset( $value_html ) ) {
                        echo innovio_mikado_get_module_part( $value_html );
					} ?>
				</ul>
				<div class="mkdf-dashboard-gallery-uploader">
					<?php
					if ( innovio_membership_theme_installed() ) {
						echo innovio_mikado_get_button_html( array(
							'text'         => esc_html__( 'Upload', 'innovio' ),
							'custom_class' => 'mkdf-dashboard-gallery-upload'
						) );
					} else {
						echo '<a itemprop="url" href="#" class="mkdf-btn mkdf-btn-medium mkdf-btn-solid mkdf-dashboard-gallery-upload"><span class="mkdf-btn-text">' . esc_html__( 'Upload', 'innovio' ) . '</span></a>';
					} ?>
					...
					<?php if ( $value !== '' && $value !== false ) { ?>
						<button class="mkdf-btn mkdf-btn-solid mkdf-dashboard-remove-image"><?php esc_html_e( 'Remove Media', 'innovio' ); ?></button>
					<?php } ?>
				</div>
			</div>
		</div>
...
class InnovioMikadoClassDashboardFieldGallery extends InnovioMikadoClassDashboardFieldType {
	public function render( $name, $label = "", $description = "", $options = array(), $args = array(), $value = '', $repeat = array() ) {
		$class = '';
...
class InnovioMikadoClassDashboardFieldSelect extends InnovioMikadoClassDashboardFieldType {
	public function render( $name, $label = "", $description = "", $options = array(), $args = array(), $value = '', $repeat = array() ) {
		$class = '';
...
class InnovioMikadoClassDashboardFieldDate extends InnovioMikadoClassFieldType {
	public function render( $name, $label = "", $description = "", $options = array(), $args = array(), $value = '', $repeat = array() ) {
		if ( ! empty( $repeat ) && array_key_exists( 'name', $repeat ) && array_key_exists( 'index', $repeat ) ) {
			$id   = $name . '-' . $repeat['index'];
			$name = $repeat['name'] . '[' . $repeat['index'] . '][' . $name . ']';
...
class InnovioMikadoClassDashboardFieldIcon extends InnovioMikadoClassDashboardFieldType {
	public function render( $name, $label = "", $description = "", $options = array(), $args = array(), $value = '', $repeat = array() ) {
		$options           = innovio_mikado_icon_collections()->getIconCollectionsEmpty();
...
class InnovioMikadoClassDashboardFieldColor extends InnovioMikadoClassDashboardFieldType {
	public function render( $name, $label = "", $description = "", $options = array(), $args = array(), $value = '', $repeat = array() ) {
		if ( ! empty( $repeat ) && array_key_exists( 'name', $repeat ) && array_key_exists( 'index', $repeat ) ) {
			$id   = $name . '-' . $repeat['index'];
			$name = $repeat['name'] . '[' . $repeat['index'] . '][' . $name . ']';
...
class InnovioMikadoClassDashboardFieldCheckBoxGroup extends InnovioMikadoClassDashboardFieldType {
	public function render( $name, $label = "", $description = "", $options = array(), $args = array(), $value = '', $repeat = array() ) {
		$col_width = 12;
...
class InnovioMikadoClassDashboardFieldAddress extends InnovioMikadoClassFieldType {
	public function render( $name, $label = "", $description = "", $options = array(), $args = array(), $value = '', $repeat = array() ) {
		$col_width = 12;
		if ( isset( $args["col_width"] ) ) {
			$col_width = $args["col_width"];
...
class InnovioMikadoClassDashboardRepeater implements iInnovioMikadoInterfaceRender {
	private $label;
	private $description;
	private $name;
	private $fields;
			...
			<?php if ( $this->table_layout ) { ?>
				<div class="mkdf-dashboard-repeater-table-heading">
					<div class="mkdf-dashboard-repeater-fields-holder">
						<div class="mkdf-dashboard-repeater-table-cell mkdf-dashboard-repeater-sort"><?php esc_html_e( 'Order', 'innovio' ) ?></div>
						...
						<div class="mkdf-dashboard-repeater-table-cell mkdf-dashboard-repeater-remove"><?php esc_html_e( 'Remove', 'innovio' ) ?></div>
					</div>
				</div>
			<?php } ?>
...
class InnovioMikadoClassDashboardFieldFactory {
	public function render( $field_type, $name, $label = "", $description = "", $options = array(), $args = array(), $value = '', $repeat = array() ) {
		switch ( strtolower( $field_type ) ) {
			case 'text':
				$field = new InnovioMikadoClassDashboardFieldText();
				$field->render( $name, $label, $description, $options, $args, $value, $repeat );
				break;
			
			case 'textarea':
				$field = new InnovioMikadoClassDashboardFieldTextArea();
				$field->render( $name, $label, $description, $options, $args, $value, $repeat );
				break;
			
			case 'date':
				$field = new InnovioMikadoClassDashboardFieldDate();
				$field->render( $name, $label, $description, $options, $args, $value, $repeat );
				break;
			
			case 'image':
				$field = new InnovioMikadoClassDashboardFieldImage();
				$field->render( $name, $label, $description, $options, $args, $value, $repeat );
				break;
			
			case 'gallery':
				$field = new InnovioMikadoClassDashboardFieldGallery();
				$field->render( $name, $label, $description, $options, $args, $value, $repeat );
				break;
			
			case 'select':
				$field = new InnovioMikadoClassDashboardFieldSelect();
				$field->render( $name, $label, $description, $options, $args, $value, $repeat );
				break;
			
			case 'icon':
				$field = new InnovioMikadoClassDashboardFieldIcon();
				$field->render( $name, $label, $description, $options, $args, $value, $repeat );
				break;
			
			case 'color':
				$field = new InnovioMikadoClassDashboardFieldColor();
				$field->render( $name, $label, $description, $options, $args, $value, $repeat );
				break;
			
			case 'checkboxgroup':
				$field = new InnovioMikadoClassDashboardFieldCheckBoxGroup();
				$field->render( $name, $label, $description, $options, $args, $value, $repeat );
				break;
			
			case 'address':
				$field = new InnovioMikadoClassDashboardFieldAddress();
