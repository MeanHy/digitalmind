...
/*
   Class: InnovioMikadoClassFramework
...
class InnovioMikadoClassFramework {
	...
	private function __construct() {
		$this->mkdOptions         = InnovioMikadoClassOptions::get_instance();
		$this->mkdMetaBoxes       = InnovioMikadoClassMetaBoxes::get_instance();
		$this->mkdTaxonomyOptions = InnovioMikadoClassTaxonomyOptions::get_instance();
		$this->mkdUserOptions     = InnovioMikadoClassUserOptions::get_instance();
		$this->mkdDashboardOptions = InnovioMikadoClassDashboardOptions::get_instance();
	}
	...
	public function setSkin( InnovioMikadoClassSkinAbstract $skinObject ) {
		$this->skin = $skinObject;
	}
}

/**
 * Class InnovioMikadoClassSkinManager
 *
...
class InnovioMikadoClassSkinManager {
	...
	/**
	 * @see InnovioMikadoClassSkinManager::setSkin()
	 */
	...
	private function setSkin( $skinName = MIKADO_PROFILE_SLUG ) {
		if ( $skinName !== '' ) {
			if ( file_exists( get_template_directory() . '/framework/admin/skins/' . $skinName . '/skin.php' ) ) {
				...
				$skinName = ucfirst( $skinName ) . esc_html__( 'Skin', 'innovio' );
...
/**
 * Class InnovioMikadoClassSkinAbstract
 *
 * Abstract class that each skin class must extend
 */
abstract class InnovioMikadoClassSkinAbstract {
	/**
	 * @var string
	 */
	protected $skinName;
	...
	public function loadTemplatePart( $template, $params = array() ) {
		...
		if ( $template !== '' ) {
			include( innovio_mikado_find_template_path( 'framework/admin/skins/' . $this->skinName . '/templates/' . $template . '.php' ) );
		}
	}
	...
	public function setShortcodeJSParams() { ?>
		<script>
			window.mkdSCIcon = '<?php echo innovio_mikado_get_skin_uri() . '/assets/img/admin-logo-icon.png'; ?>';
			window.mkdSCLabel = '<?php echo esc_html( ucfirst( $this->skinName ) ); ?> <?php esc_html_e( 'Shortcodes', 'innovio' ) ?>';
		</script>
	<?php }
...
/*
   Class: InnovioMikadoClassOptions
...
class InnovioMikadoClassOptions {
	...
	public function getOptionValue( $key ) {
		...
		if ( array_key_exists( $key, $innovio_mikado_global_options ) ) {
			return $innovio_mikado_global_options[ $key ];
...
/*
   Class: InnovioMikadoClassAdminPage
   A class that initializes Mikado Admin Page
*/
class InnovioMikadoClassAdminPage implements iInnovioMikadoInterfaceLayoutNode {
	public $layout;
	private $factory;
	public $slug;
	public $title;
	...
	function __construct( $slug = "", $title_label = "", $icon = "" ) {
		$this->layout  = array();
		$this->factory = new InnovioMikadoClassFieldFactory();
		$this->slug    = $slug;
	...
	public function renderChild( iInnovioMikadoInterfaceRender $child ) {
		$child->render( $this->factory );
	}
}

/*
   Class: InnovioMikadoClassMetaBoxes
...
/*
   Class: InnovioMikadoClassMetaBox
   A class that initializes Mikado Meta Box
*/
class InnovioMikadoClassMetaBox implements iInnovioMikadoInterfaceLayoutNode {
	public $layout;
	private $factory;
	public $scope;
	public $title;
	...
	public function setTitle( $label ) {
		...
		return $innovio_mikado_global_Framework->getSkin()->getSkinLabel() . ' ' . $label;
	}
}

/*
   Class: InnovioMikadoClassTaxonomyOptions
   A class that initializes InnovioMikadoClass Taxonomy Options
*/
class InnovioMikadoClassTaxonomyOptions {
	private static $instance;
...
/*
   Class: InnovioMikadoClassTaxonomyOption
   A class that initializes InnovioMikadoClass Taxonomy Option
*/
class InnovioMikadoClassTaxonomyOption implements iInnovioMikadoInterfaceLayoutNode {
	public $layout;
	private $factory;
	public $scope;
	...
	function __construct( $scope = "" ) {
		$this->layout  = array();
		$this->factory = new InnovioMikadoClassTaxonomyFieldFactory();
		$this->scope   = $scope;
	}
...
/*
   Class: InnovioMikadoClassUserOptions
   A class that initializes InnovioMikadoClass User Options
*/
class InnovioMikadoClassUserOptions {
	private static $instance;
	public $userOptions;
...
/*
   Class: InnovioMikadoClassUserOption
   A class that initializes InnovioMikadoClass User Option
*/
class InnovioMikadoClassUserOption implements iInnovioMikadoInterfaceLayoutNode {
	public $layout;
	private $factory;
	public $scope;
	...
	function __construct( $scope = "" ) {
		$this->layout  = array();
		$this->factory = new InnovioMikadoClassUserFieldFactory();
		$this->scope   = $scope;
	}
...
/*
   Class: InnovioMikadoClassDashboardOptions
   A class that initializes InnovioMikadoClass Dashboard Options
*/
class InnovioMikadoClassDashboardOptions {
	private static $instance;
...
/*
   Class: InnovioMikadoClassDashboardOption
   A class that initializes InnovioMikadoClass Dashboard Option
*/
class InnovioMikadoClassDashboardOption implements iInnovioMikadoInterfaceLayoutNode {
	public $layout;
	private $factory;
	
	function __construct() {
		$this->layout        = array();
		$this->factory       = new InnovioMikadoClassDashboardFieldFactory();
	}
...
if ( ! function_exists( 'innovio_mikado_init_framework_variable' ) ) {
	function innovio_mikado_init_framework_variable() {
		global $innovio_mikado_global_Framework;
		
		$innovio_mikado_global_Framework = InnovioMikadoClassFramework::get_instance();
		$mkdSkinManager                 = new InnovioMikadoClassSkinManager();
		$innovio_mikado_global_Framework->setSkin( $mkdSkinManager->getSkin() );
	}
	
	add_action( 'innovio_mikado_action_before_options_map', 'innovio_mikado_init_framework_variable' );
}
?>