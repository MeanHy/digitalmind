...
class InnovioMikadoClassLike {
	private static $instance;
	
	private function __construct() {
		add_action( 'wp_ajax_innovio_mikado_like', array( $this, 'ajax' ) );
		add_action( 'wp_ajax_nopriv_innovio_mikado_like', array( $this, 'ajax' ) );
	}
	...
	public function like_post( $post_id, $action = 'get', $type = '' ) {
		...
		switch ( $action ) {
			...
			case 'get':
				...
				$return_value = $icon . "<span>" . esc_attr__('like','innovio') . "</span>";
			...
			case 'update':
				...
				$return_value = "<i class='icon_heart' aria-hidden='true'></i><span>" . esc_attr__('like','innovio') . "</span>";
	...
	public function add_like() {
		...
		$title       = esc_html__( 'Like this', 'innovio' );
		
		if ( isset( $_COOKIE[ 'mkdf-like_' . $post->ID ] ) ) {
			$class = 'mkdf-like liked';
			$title = esc_html__( 'You already like this!', 'innovio' );
		}
...
if ( ! function_exists( 'innovio_mikado_create_like' ) ) {
	function innovio_mikado_create_like() {
		InnovioMikadoClassLike::get_instance();
	}
	
	add_action( 'after_setup_theme', 'innovio_mikado_create_like' );
}