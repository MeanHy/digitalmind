 ...
 */
if ( ! class_exists( 'InnovioMikadoClassSidebar' ) ) {
	class InnovioMikadoClassSidebar {
		var $sidebars = array();
		var $stored = "";
		...
		function __construct() {
			...
			$this->title  = esc_html__( 'Custom Widget Area', 'innovio' );
		...
		function load_assets() {
			...
			wp_enqueue_script( 'innovio-mikado-admin-sidebar-script', MIKADO_ROOT . '/framework/admin/assets/js/mkdf-ui/mkdf-sidebar.js' );
			wp_enqueue_style( 'innovio-mikado-admin-sidebar', MIKADO_ROOT . '/framework/admin/assets/css/mkdf-sidebar.css' );
		}
		...
		function template_add_widget_field() {
			...
			echo "\n    <span class='input_wrap'><input type='text' value='' placeholder = '" . esc_attr__('Enter Name of the new Widget Area', 'innovio') . "' name='mkdf-add-widget' /></span>";
			echo "\n    <input class='button' type='submit' value='" . esc_attr__('Add Widget Area', 'innovio') . "' />";
			echo "\n    " . $nonce;
			echo "\n  </form>";
			echo "\n</script>\n";
		}
		...
		function register_custom_sidebars() {
			...
			$args = apply_filters( 'innovio_mikado_filter_custom_widget_args', $args );
