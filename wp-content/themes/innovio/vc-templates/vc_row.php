...
if (!empty($enable_grid_lines)){
    $css_classes[] = 'mkdf-' . esc_attr($enable_grid_lines);
}

if ($grid_lines_skin === 'grid_line_light'){
    $css_classes[] = 'mkdf-grid-lines-light';
}
...
if ( $row_content_width !== 'grid' ) {
	...
	if ( ! empty( $simple_background_image ) ) {
		...
		$image = is_array($image_src)? $image_src[0] : '';
		$mkdf_vc_row_style[] = 'background-image: url(' . esc_url( $image ) . ')';
	}
	...
	if ( ! empty( $simple_background_image ) ) {
		$image_src              = wp_get_attachment_image_src( $simple_background_image, 'full' );
		$image2 = is_array($image_src)? $image_src[0] : '';
		$mkdf_grid_row_style[] = 'background-image: url(' . esc_url( $image2 ) . ')';
	}
...
if ( $row_content_width === 'grid' ) {
	$mkdf_row_wrapper_start .= '<div class="mkdf-row-grid-section-wrapper ' . esc_attr( $grid_row_classes ) . '" ' . esc_attr( $grid_row_datas ) . ' ' . innovio_mikado_get_inline_style( $mkdf_grid_row_styles ) . '><div class="mkdf-row-grid-section">';
	$mkdf_row_wrapper_end   .= '</div></div>';
}

$mkdf_grid_row_parallax_grids = '';
if( $enable_grid_lines === 'grid_line_right' ) {
    $mkdf_grid_row_parallax_grids = '<span class="mkdf-grid-line-right"><span class="mkdf-grid-line-inner" data-parallax="{&quot;y&quot;:-450, &quot;smoothness&quot;:25}"></span></span>';
}

else if( $enable_grid_lines === 'grid_line_dual' ) {
    $mkdf_grid_row_parallax_grids = '<span class="mkdf-grid-line-left"><span class="mkdf-grid-line-inner" data-parallax="{&quot;y&quot;:750, &quot;smoothness&quot;:25}"></span></span><span class="mkdf-grid-line-center-left"><span class="mkdf-grid-line-inner" data-parallax="{&quot;y&quot;:750, &quot;smoothness&quot;:25}"></span></span><span class="mkdf-grid-line-center-right"><span class="mkdf-grid-line-inner" data-parallax="{&quot;y&quot;:750, &quot;smoothness&quot;:25}"></span></span><span class="mkdf-grid-line-right"><span class="mkdf-grid-line-inner" data-parallax="{&quot;y&quot;:-450, &quot;smoothness&quot;:25}"></span></span>';
}

$mkdf_after_wrapper_open   = apply_filters('innovio_mikado_filter_vc_after_wrapper_open', '', $atts);
$mkdf_before_wrapper_close = apply_filters('innovio_mikado_filter_vc_before_wrapper_close', '', $atts);
$css_classes                = apply_filters('innovio_mikado_filter_vc_css_classes', $css_classes, $atts);
...
$output .= '<div ' . implode( ' ', $wrapper_attributes ) . ' ' . innovio_mikado_get_inline_style( $mkdf_vc_row_styles ) . '>';
$output .= $mkdf_after_wrapper_open;
$output .= wpb_js_remove_wpautop( $content );
$output .= $mkdf_grid_row_parallax_grids;
$output .= $mkdf_before_wrapper_close;
$output .= '</div>';
$output .= $mkdf_row_wrapper_end;
$output .= $after_output;

echo innovio_mikado_get_module_part( $output );
